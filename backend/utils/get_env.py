...
def get_app_data_directory_env():
    val = os.getenv("APP_DATA_DIRECTORY")
    if val:
        return val
    fastapi_root = os.path.abspath(os.path.join(os.path.dirname(__file__), ".."))
    default_dir = os.path.join(fastapi_root, "app_data")
...
def get_user_config_path_env():
    val = os.getenv("USER_CONFIG_PATH")
    if val:
        return val
    fastapi_root = os.path.abspath(os.path.join(os.path.dirname(__file__), ".."))
    default_dir = os.path.join(fastapi_root, "app_data")
    os.makedirs(default_dir, exist_ok=True)
    return os.path.join(default_dir, "user-config.json")
...
def is_presenton_electron_desktop():
    """True when running inside the PitchAI Electron desktop app."""
