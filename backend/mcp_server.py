...
class PitchAITokenVerifier(TokenVerifier):
    """Validate PitchAI session tokens for MCP HTTP auth."""
...
async def main():
    try:
        if not is_mcp_server_enabled():
            print(
                "INFO: MCP server is disabled in the PitchAI Electron desktop app "
