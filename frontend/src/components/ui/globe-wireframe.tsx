...
interface GlobeWireframeProps {
  ...
  connections?: { start: [number, number], end: [number, number] }[];
  ...
  useEffect(() => {
    ...
    if (connections && connections.length > 0) {
      ...
      svg.selectAll('.connection')
        ...
        .attr('stroke-dashoffset', rotation[0] * -2)
        .attr('opacity', 0.8)
        .style('visibility', function(this: SVGPathElement) {
           const pathData = d3.select(this).attr('d');
    ...
    if (locations && locations.length > 0) {
      const g = svg.selectAll('.location-pin')
        .data(locations)
        .enter()
        .append('g')
        .attr('class', 'location-pin')
        .style('visibility', (d) => {
           const pointFeature = { type: 'Feature', geometry: { type: 'Point', coordinates: [d[1], d[0]] } };
           const pathData = path(pointFeature as any);
           return pathData && pathData.length > 0 && !pathData.includes('NaN') ? 'visible' : 'hidden';
        })
        .attr('transform', (d) => {
           const coords = (projection as d3.GeoProjection)([d[1], d[0]]);
           if (coords) {
             const bounceY = Math.sin(Date.now() / 150) * 4;
             return `translate(${coords[0] - 12}, ${coords[1] - 22 + bounceY})`;
           }
           return '';
        });
