...
export default function Footer({ bgClass = "bg-black" }: { bgClass?: string }) {
  return (
    <footer className={`site-footer ${bgClass} text-white pb-10 border-t border-zinc-900 relative overflow-hidden`}>
      ...
      <div className="container mx-auto px-6 md:px-12 max-w-7xl relative z-10 pt-20">
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-12 lg:gap-8 mb-20">
          <div className="col-span-1 md:col-span-2 lg:col-span-2">
            <h2 className="text-3xl md:text-4xl font-serif tracking-tight text-white/90 mb-4 max-w-sm">
          ...
          <div className="flex flex-col gap-4">
            <h3 className="text-xs font-bold uppercase tracking-widest text-zinc-500 mb-2">Platform</h3>
            <Link href="/upcoming-hackathons" className="text-zinc-300 hover:text-white transition-colors text-sm font-medium">Hackathons</Link>
            <Link href="/resources" className="text-zinc-300 hover:text-white transition-colors text-sm font-medium">Resources</Link>
            <Link href="/coach" className="text-zinc-300 hover:text-white transition-colors text-sm font-medium">AI Coach</Link>
          </div>

          <div className="flex flex-col gap-4">
            <h3 className="text-xs font-bold uppercase tracking-widest text-zinc-500 mb-2">Community</h3>
            <a href="https://github.com" target="_blank" rel="noreferrer" className="text-zinc-300 hover:text-white transition-colors text-sm font-medium">GitHub</a>
            <a href="https://x.com" target="_blank" rel="noreferrer" className="text-zinc-300 hover:text-white transition-colors text-sm font-medium">Twitter / X</a>
            <a href="https://linkedin.com" target="_blank" rel="noreferrer" className="text-zinc-300 hover:text-white transition-colors text-sm font-medium">LinkedIn</a>
          </div>
        </div>
