...
export function SearchNodesModal({
  ...
  return (
    <div className="fixed inset-0 z-50 bg-slate-900/40 backdrop-blur-xs flex items-start justify-center pt-24 p-4">
        ...
        <div className="p-2 max-h-64 overflow-auto space-y-1">
          {filtered.map((item, idx) => (
            <div
              ...
              className="p-2.5 rounded-xl hover:bg-blue-50/70 hover:text-blue-700 cursor-pointer flex items-center justify-between transition-colors text-xs font-medium text-slate-700"
            >
      ...
      <div className="bg-white rounded-2xl shadow-2xl border border-slate-200 w-full max-w-md overflow-hidden">
        <div className="p-4 border-b border-slate-100 flex items-center justify-between bg-slate-50/50">
          <div className="flex items-center gap-2">
            ...
            <h3 className="text-sm font-bold text-slate-900">Control Center Settings</h3>
          </div>
          <button onClick={onClose} className="p-1 text-slate-400 hover:text-slate-700">
            <X className="size-4" />
          </button>
        ...
        <div className="p-5 space-y-4 text-xs">
          <div>
            <label className="font-bold text-slate-700 block mb-1">Agent Concurrency Speed</label>
            <select
              value={speed}
          ...
          <div>
            <label className="font-bold text-slate-700 block mb-1">FastMCP Upstream Timeout</label>
            <select
              value={timeout}
          ...
          <div className="flex items-center justify-between p-3 rounded-xl bg-slate-50 border border-slate-100">
            <div>
              <p className="font-bold text-slate-800">Auto-Retry Failed Tool Calls</p>
              <span className="text-[10px] text-slate-400">Retry transient API timeouts automatically</span>
            </div>
            <input
              type="checkbox"
              checked={autoRetry}
          ...
          <button
            onClick={onClose}
            className="w-full py-2 bg-blue-600 text-white font-bold rounded-xl shadow-md shadow-blue-500/20"
          >
      ...
      <div className="bg-white rounded-2xl shadow-2xl border border-slate-200 w-full max-w-md overflow-hidden">
        <div className="p-4 border-b border-slate-100 flex items-center justify-between bg-slate-50/50">
          <div className="flex items-center gap-2">
            ...
            <h3 className="text-sm font-bold text-slate-900">AI Agents Fleet Monitor</h3>
          </div>
          <button onClick={onClose} className="p-1 text-slate-400 hover:text-slate-700">
            <X className="size-4" />
          </button>
            ...
            <div key={idx} className="p-3 rounded-xl border border-slate-200 flex items-center justify-between">
              <div className="flex items-center gap-2.5">
                ...
                <div>
                  ...
                  <span className="text-[10px] text-slate-400">Load: {a.load}</span>
                </div>
              </div>
      ...
      <div className="bg-white rounded-2xl shadow-2xl border border-slate-200 w-full max-w-md overflow-hidden">
        <div className="p-4 border-b border-slate-100 flex items-center justify-between bg-slate-50/50">
          <div className="flex items-center gap-2">
            ...
            <h3 className="text-sm font-bold text-slate-900">Connected Services & Integrations</h3>
          </div>
          <button onClick={onClose} className="p-1 text-slate-400 hover:text-slate-700">
            <X className="size-4" />
          </button>
      ...
      <div className="bg-white rounded-2xl shadow-2xl border border-slate-200 w-full max-w-md overflow-hidden">
        <div className="p-4 border-b border-slate-100 flex items-center justify-between bg-slate-50/50">
          <div className="flex items-center gap-2">
            ...
            <h3 className="text-sm font-bold text-slate-900">System & Workspace Settings</h3>
          </div>
          <button onClick={onClose} className="p-1 text-slate-400 hover:text-slate-700">
            <X className="size-4" />
          </button>
        ...
        <div className="p-5 space-y-3 text-xs">
          <div className="p-3 rounded-xl bg-slate-50 border border-slate-100 flex items-center justify-between">
            <span className="font-bold text-slate-700">Environment Mode</span>
          ...
          <div className="p-3 rounded-xl bg-slate-50 border border-slate-100 flex items-center justify-between">
            <span className="font-bold text-slate-700">FastAPI API Target</span>
            <span className="font-mono bg-white px-2 py-0.5 rounded border border-slate-200 text-slate-800">
              http://localhost:8000
            </span>
          </div>
      ...
      <div className="bg-white rounded-2xl shadow-2xl border border-slate-200 w-full max-w-md overflow-hidden">
        <div className="p-4 border-b border-slate-100 flex items-center justify-between bg-slate-50/50">
          <div className="flex items-center gap-2">
            ...
            <h3 className="text-sm font-bold text-slate-900">Help Desk & Canvas Shortcuts</h3>
          </div>
          <button onClick={onClose} className="p-1 text-slate-400 hover:text-slate-700">
            <X className="size-4" />
          </button>
        ...
        <div className="p-5 space-y-3 text-xs">
          <div className="flex items-center justify-between">
            <span className="text-slate-600 font-medium">Search Nodes & Tools</span>
            <kbd className="px-2 py-0.5 bg-slate-100 font-mono rounded border text-slate-700">⌘K</kbd>
          </div>
          <div className="flex items-center justify-between">
            <span className="text-slate-600 font-medium">Test Automation Pipeline</span>
            <kbd className="px-2 py-0.5 bg-slate-100 font-mono rounded border text-slate-700">Alt + R</kbd>
          </div>
          <div className="flex items-center justify-between">
            <span className="text-slate-600 font-medium">Add New Canvas Node</span>
            <kbd className="px-2 py-0.5 bg-slate-100 font-mono rounded border text-slate-700">N</kbd>
          </div>
          <div className="flex items-center justify-between">
            <span className="text-slate-600 font-medium">Zoom In / Zoom Out</span>
            <kbd className="px-2 py-0.5 bg-slate-100 font-mono rounded border text-slate-700">+</kbd>
          </div>
        </div>
      </div>
    </div>
...
export function UserProfileModal({
  ...
  return (
    <div className="fixed inset-0 z-50 bg-slate-900/40 backdrop-blur-xs flex items-center justify-center p-4">
      <div className="bg-white rounded-2xl shadow-2xl border border-slate-200 w-full max-w-sm overflow-hidden text-center p-6 space-y-4">
        <div className="size-16 mx-auto rounded-2xl bg-blue-600 text-white font-bold text-xl flex items-center justify-center shadow-lg shadow-blue-500/30">
          HK
        </div>
        <div>
          <h3 className="text-base font-bold text-slate-900">HAC-KIT Admin</h3>
          <p className="text-xs text-slate-400">System Operator & Hackathon Builder</p>
        </div>
        <div className="p-3 rounded-xl bg-slate-50 border border-slate-100 text-xs text-slate-600 space-y-1">
          <p>Session ID: <span className="font-mono text-slate-900">hk-admin-2026</span></p>
