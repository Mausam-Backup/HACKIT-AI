...
const PitchAIMode = ({
                ...
                <div className="space-y-4">
                    {selectedWebProvider.value === "auto" && (
                        <div className="rounded-lg border border-[#D9D6FE] bg-[#F4F3FF] p-3 text-xs leading-5 text-[#5146E5]">
                            PitchAI will use model-native web grounding when available. If the selected text model does not support it, web search stays off until you choose an external provider.
                        </div>
                    )}
    ...
    return (
        <div className='w-full max-w-[660px] font-syne pb-10'>
            <p className='px-2.5 py-0.5 w-fit text-[#7A5AF8] rounded-[50px]  border border-[#EDEEEF] text-[10px] font-medium mb-5 font-syne'>PITCHAI</p>
            <div className=''>
                ...
                <p className='text-[#000000CC] text-xl font-normal font-syne'>
                    {providerStep === 1
                        ...
                        : providerStep === 2
                            ? "Choose how PitchAI creates visuals, or continue without image generation."
                            ...
                            <PopoverTrigger asChild>
                                ...
                                >
                                    <div className="flex gap-3 items-center">
                                        {llmConfig.LLM && OTHER_PROVIDER_VALUES.has(llmConfig.LLM) && LLM_PROVIDERS[llmConfig.LLM]?.icon && (
                                            <img src={LLM_PROVIDERS[llmConfig.LLM]?.icon} alt="" className="w-4 h-4 object-contain shrink-0" />
                                        )}
                                            ...
                                            {OTHER_PROVIDERS.map(
                                                (provider, index) => (
                                                    ...
                                                    >
                                                        ...
                                                        <div className="flex gap-3 items-center">
                                                            {provider.icon && (
                                                                <img src={provider.icon} alt={provider.label} className="w-5 h-5 object-contain shrink-0" />
                                                            )}
