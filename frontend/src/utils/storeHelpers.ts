...
export const getLLMConfigValidationError = (
  ...
  if (llm === "openai") {
    ...
    if (!isProvided(llmConfig.GROQ_API_KEY)) {
      return "Groq API key is required.";
    }
    if (!isProvided(llmConfig.GROQ_MODEL)) {
      return 'No Groq model selected. Use "Check models" after entering your API key, then choose a model.';
    }
  } else if (llm === "vertex") {
