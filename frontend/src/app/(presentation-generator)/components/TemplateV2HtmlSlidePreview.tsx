...
declare global {
  interface Window {
    __PRESENTON_JSON_CHARTS__?: PitchAIChartGlobalState;
  }
}

const useChartLayoutEffect =
  typeof window === "undefined" ? useEffect : useLayoutEffect;
let didRegisterPitchAIDataLabelPlugin = false;

function registerPitchAIDataLabelPlugin(): void {
  if (didRegisterPitchAIDataLabelPlugin) return;
  Chart.register(presentonDataLabelPlugin);
  didRegisterPitchAIDataLabelPlugin = true;
}

function chartGlobalState(): PitchAIChartGlobalState | null {
  if (typeof window === "undefined") return null;

  const existing = window.__PRESENTON_JSON_CHARTS__;
  if (existing) return existing;

  const state: PitchAIChartGlobalState = {
    status: "ready",
    pending: 0,
    rendered: 0,
  };
...
const presentonDataLabelPlugin: Plugin = {
  ...
  afterDatasetsDraw(chart, _args, options) {
    const labelOptions = options as PitchAIDataLabelOptions | undefined;
...
export function TemplateV2HtmlSlidePreview({
  ...
  useChartLayoutEffect(() => {
    ...
    const renderCharts = () => {
      ...
      try {
        registerPitchAIDataLabelPlugin();
